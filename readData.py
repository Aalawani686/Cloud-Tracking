...
def trackContours(tm, lastContours, contours, hierarchy, wX, wY, image, isFirst):
    ...
    # image = cv2.circle(image, ((int)(image.shape[0]/2), (int)(image.shape[1]/2)), 1, (0, 0, 255), 2)
    ...
    for c in range(len(clouds)):
        ...
        if ((isFirst and dist > 0) or clouds[c].selected) :
            ...
            global radii
            radii[0].append(times[tm])
            radii[1].append(radius)
            global distances
            distances[0].append(times[tm])
            distances[1].append(minDistance)
...
# def timeToString(ts):
#     return "%s:%s:%s" % (str(ts//10000), str(ts%10000//100), str(ts%100))
...
def plotCloudAndVelocity(cloud_id, clouds):
    ...
    plt.ylabel('Height (m)')
    plt.xlabel('Time (h)')
    plt.scatter(time_plot, height_plot, zorder = 1)
    plt.subplot(212)
    ...
    def draw(time):
        ...
        ax.add_patch(Rectangle((time - xrange/40, someY - 3000), xrange/20, 6000, facecolor="yellow", zorder = 0))

        xrange_main = main.get_xlim()[1] - main.get_xlim()[0]
        main.add_patch(Rectangle((hoursToSeconds(time) - xrange_main/80, someY - 3000), xrange_main/40, 6000, facecolor="black", zorder = 0))
